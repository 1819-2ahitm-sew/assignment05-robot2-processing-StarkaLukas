...
public class Robot2{
    ...
    public void stepForward(){
        ...
        if (mode == Mode.TELEPORT) {
            if (x < 0) {
                x = board.getFieldBox()[0].length - 1;
            } else if(x >= board.getFieldBox()[0].length){
                x = 0;
            }
            else if (y < 0) {
                y = board.getFieldBox().length - 1;
            } else if (y >= board.getFieldBox().length) {
                y = 0;
            }
        }
            ...
            if (y < 0) {
                y = 0;
            } else if (y >= board.getFieldBox()[0].length) {
                y = board.getFieldBox()[0].length - 1;
            }
        }
